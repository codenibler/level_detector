...
def find_next_resistance(current_level, levels_df):
    ...
    if not higher_levels.empty:
        return higher_levels['level'].min()
    return None
...
for candle in tqdm(data.itertuples(), desc="Crunching Backtest, Dawg", unit="candle", total=len(data)):
    ...
    if timestamp.hour == 00 and timestamp.minute == 00:
        ...
        # data_window = data[:candle.Index]
        # data_30m = data_window.resample('30min').agg({'open':'first', 'high':'max', 'low':'min', 'close':'last', 'volume':'sum'}).dropna().reset_index()
    ...
    for level in levels.itertuples():
        if level.level_type == 'resistance': # Looking for short. 
            if high > level.level and low <= level.level:
                entry_resistance = find_next_resistance(level.level, levels)
                ...
                if ((timestamp.date(), level.level) in daily_used_levels):
                    continue
                daily_used_levels.append((timestamp.date(), level.level))
                pending_trade = PendingTrade(
                    level_type='resistance',
                    level=level.level,
                    timestamp=timestamp,
                    entry=entry_resistance,
                    tp= entry_resistance - take_profit_distance,
                    sl= entry_resistance + take_profit_distance
                )
                pending_trades.append(pending_trade)

        elif level.level_type == 'support': # Looking for long.
            if low < level.level and high >= level.level:
                entry_support = find_next_support(level.level, levels)
                ...
                pending_trade = PendingTrade(
                    ...
                    timestamp=timestamp, 
                    entry=entry_support,
                    tp= entry_support + take_profit_distance,
                    sl= entry_support - take_profit_distance
                )
                pending_trades.append(pending_trade)
        ...
        if trade.level_type == 'resistance':
            if low <= trade.tp:
                closed_trades.append(ClosedTrade(
                    ...
                    sl=trade.sl,
                    outcome='tp',
                    pnl= (trade.entry_level - SPREAD) - (trade.tp + SPREAD)
                ))
                ...
                closed_trades.append(ClosedTrade(
                    ...
                    sl=trade.sl,
                    outcome='sl',
                    pnl= -((trade.sl + SPREAD) - (trade.entry_level - SPREAD))
                ))
                open_trades.remove(trade)
        ### CLOSING LONG TRADES
            ...
            if high >= trade.tp:
                closed_trades.append(ClosedTrade(
                    ...
                    sl=trade.sl,
                    outcome='tp',
                    pnl= (trade.tp - SPREAD) - (trade.entry_level + SPREAD)
                ))
                open_trades.remove(trade)
            elif low <= trade.sl:
                closed_trades.append(ClosedTrade(
                    ...
                    sl=trade.sl,
                    outcome='sl',
                    pnl=-((trade.entry_level + SPREAD)  - (trade.sl - SPREAD))
                ))
                open_trades.remove(trade)
